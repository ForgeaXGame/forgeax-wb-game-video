...
|------------------|-------------------------------------|----------------------------------------------|------------|
| **shared**       | `kubeela-marketplace/src/memory/`   | Project-wide context: "what is Kubeela", "current sprint focus", "known gotchas" | maintainer commits |
| **per-instance** | `kubeela/.kubeela/agenteam-state/instances/<id>/team/agents/<id>/memory/` | What this specific agent learned: user prefs, project history, recurring decisions | the agent itself, via the `memory` tool |

The cli's `marketplace_loader` capability (Phase 2 — see issue
[#kubeela-marketplace-wire-up]) loads **shared** memory at instance
...
```
src/memory/
├── 00-kubeela-overview.md      project-wide context
...
- **Don't store per-game design** here. Game design goes under
  `kubeela/games/<slug>/` (the workspace), not into agent memory.
