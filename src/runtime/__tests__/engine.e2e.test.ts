...
describe('nodia graph e2e (runs on GraphRuntime)', () => {
  ...
  it('quick win: light skill kills a low-hp boss → flow ends (no forced banner)', () => {
    ...
    rt.jumpToNode('a_my') // 跳过叙事，从我方回合容器进入 wait（保留初始全局态）
  ...
  it('turn loop: 我方先手一整回合(我方攻击→敌方回合)存活 → 回到我方回合待机 (回合循环成立)', () => {
    ...
    rt.onPerformanceEnd() // returns → b_ai 回合结束判定(双方存活+我方先手) → a_my → wait
