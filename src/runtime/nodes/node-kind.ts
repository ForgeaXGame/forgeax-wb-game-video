 ...
 */
import type { ElementRole, GameGraph, GameNode } from '../schema/graph-schema'
 ...
 * 调用栈非空则 pop 弹回 caller、栈空则 `finishEnd` 结束本局。能走到哪就停在哪。
 */
export type NextIntent =
  | { kind: 'advance' }                                 // 沿默认出边推进（无边：有栈则弹回 caller，栈空则结束本局）
  | { kind: 'await' }                                   // 挂起：等交互/媒体/时钟（相位已由元素运行设好）
  | { kind: 'descend'; entry: string; graph?: GameGraph } // 下钻子流程(同图)/子蓝图(带 graph 切图)

/**
 * 引擎提供给 NodeKind 的受控上下文。节点通过这里发指令/跑元素/读调度态，
 * 不能直接改 callStack/phase/redirect。内置 kind 的副作用一律经 runElement / applyEnterReactions
 * 走既有写屏障（applyAndReact → checkRules/checkWatch），故引擎不在 execute 外围额外 checkRules
 * （避免因 `visited.add` 等入场副作用让 state 规则比旧逻辑更早命中）。将来若加「直接改 state 的自定义
 * 节点」，应在此暴露一个带屏障的 mutate 方法，而非在调度层无条件 checkRules。
 */
...
export interface NodeKind {
  type: string
  /** 初始化 + 执行 + 首个走向意图。同步节点一次出结果；perf 有 media/交互返回 { kind:'await' }。 */
 ...
 * 以后往 `GameNode.type` 加真类型时，无 subFlow* 的节点直接按 type 派发，叠加式、老图零影响。
 */
