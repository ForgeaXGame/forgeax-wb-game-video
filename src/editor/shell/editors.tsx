...
export function EffectsEditor({
  ...
  return (
    <div>
      ...
      <button style={{ marginTop: 4 }} onClick={() => onChange([...list, defaultEffect('attr', entities, variables)])}>+ effect</button>
    </div>
  )
}
...
export function ConditionEditor({
  ...
  return (
    <div>
      ...
      <button style={{ marginTop: 4 }} onClick={() => set([...all, defaultClause('attrRatio', entities, variables)])}>+ 条件（AND）</button>
    </div>
  )
}
