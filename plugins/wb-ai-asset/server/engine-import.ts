// Manual "Import to Game" for wb-ai-asset props (PLAN §5.3 / AI1 / AI2 / DRACO1).
...
import type { WbAssetMeta } from '../shared/manifest';
...
export async function engineImportStatus(
  ...
  const { glbAbs, metaAbs, wbAbs } = resolved;
  ...
  let wb: WbAssetMeta | null = null;
  if (await exists(wbAbs)) {
    try {
      wb = JSON.parse(await readFile(wbAbs, 'utf8')) as WbAssetMeta;
    } catch {
      wb = null;
    }
  }
  const engineImport = (wb?.custom as { engineImport?: { importedAt?: string; sourceHash?: string } } | undefined)
    ?.engineImport;
  ...
  if (hasMeta && recordedHash === currentHash) {
    return {
      ...
      importedAt,
      message: 'Already imported to game (engine meta matches current GLB).',
      retryable: false,
    };
  }
  ...
  return {
    ...
    imported: false,
    needsManualImport: true,
    needsDracoNormalize: needsDraco,
    engineMetaPath: hasMeta ? metaAbs : null,
    sourceHash: currentHash,
    importedAt,
    message: needsDraco
...
export async function importToEngine(
  ...
  const { glbAbs, metaAbs, wbAbs, glbFileName } = resolved;
  if (!(await exists(glbAbs))) {
  ...
  if (
    ...
    if (!normalized.ok) {
      return {
        ...
        message: `Draco normalize failed: ${normalized.message}`,
        retryable: true,
      };
    }
  ...
  if (await exists(wbAbs)) {
    try {
      const wb = JSON.parse(await readFile(wbAbs, 'utf8')) as WbAssetMeta;
      wb.contentHash = contentHash;
      wb.size = glbBytes.byteLength;
      (wb.custom as Record<string, unknown>).engineImport = {
        sourceHash: contentHash,
        importedAt: new Date().toISOString(),
      };
      await writeFile(wbAbs, `${JSON.stringify(wb, null, 2)}\n`, 'utf8');
    } catch {
  ...
  return {
    ...
    assetPath,
    message:
      'Imported to game Edit asset catalog. This does not auto-replace the game protagonist or modify gameplay code.',
  };
}
