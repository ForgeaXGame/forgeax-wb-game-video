...
//
// The gateway task endpoint returns outputs as a `data[]` array of
// { url, type, format } entries. Two quirks live here so provider parsing can't
// drift:
//
...
export function extractGatewayUrls(resp: Record<string, unknown>): Record<string, string> {
  ...
  for (const raw of data) {
    ...
    if (/^preview\./i.test(name)) {
      ...
      // A PBR texture map — never the thumbnail. base_color feeds the texture
      // sidefile; the rest are matched by the provider's TEXTURE_KINDS.
      out[`__texture_${textureKindFromName(name)}`] = url;
    } else if (type === 'texture') {
      // A gateway that tags textures correctly with an explicit texture_kind.
