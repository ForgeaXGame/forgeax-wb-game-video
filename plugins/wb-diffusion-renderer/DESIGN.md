...
Files: `packages/marketplace/plugins/wb-diffusion-renderer/src/panel.tsx` and
...
- **Phase 2 — real-time streaming. ✅ BUILT + VERIFIED (2026-07-06).** WS relay at
  **`/ws/diffusion-renderer?backend=<name>`** (under `/ws/*` so the dev vite proxy — which enables
  ...
  `marketplace/plugins/wb-diffusion-renderer/src/host/stream.ts` + `host/encode-worker.ts`;
...
- `serveStatic('/plugins/wb-diffusion-renderer/*')` route in `packages/server/src/main.ts`.
```
