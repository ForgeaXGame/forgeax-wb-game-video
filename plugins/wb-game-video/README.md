...
**只有 graph 引擎一套**（`src/blueprint/graph/`）—— 蓝图编辑 / 试玩 / 视频 / 界面 / 规则。

- 一张 `GameScenario` 图：**只有「演出节点」（每个绑视频）**，判断（出手/血量/胜负/变招…）折进**条件/加权出边**，无独立网关节点。
- **SSOT = 插件内只读 `src/blueprint/graph/demo/nodia.graph.json`**；主动保存的版本落盘到 `.forgeax/games/<slug>/game-video/scenarios.graph.json`（+ 版本快照），未保存草稿走 localStorage，「重置」回 demo。
- 引擎纯 TS、零 DOM、种子 RNG；一切逻辑声明式可序列化、无函数入库。
- 盖在视频上的 QTE/血条/选择等 = `src/blueprint/graph/react/skins/` 下**自闭环、可替换、带错误边界**的组件，图里只记 `component` id。
...
- 硬性规则（Schema / Runtime / 持久化 / 皮肤组件）见 **[`AGENTS.md`](./AGENTS.md)**；组件契约见 **[`skins/CONTRACT.md`](./src/blueprint/graph/react/skins/CONTRACT.md)**；AI 调用见 **[`SKILL.md`](./SKILL.md)**。
...
| 皮肤/HUD/QTE 组件契约（props/submit/自闭环/错误隔离） | [`src/blueprint/graph/react/skins/CONTRACT.md`](./src/blueprint/graph/react/skins/CONTRACT.md) |
