 ...
 *   用户在内置 demo 上锻造新剧本 → 新剧本被烙成 demo-001, 把内置样板从磁盘
 ...
 *   2. 如果它的 scenario **不是**内置样板 (title / scene 结构不匹配)
 ...
 *      (真正属于 demo-001 的资产保留, 因为内置样板自己也是 demo-001;
 *       这里靠 sceneId 区分: 内置样板的 scene 是当前 getDemoScenario() 的 sceneIds,
...
/**
 * 内置样板的所有 sceneId. 用于在 manifest.json 里区分:
...
export function migrateBuiltinDemoIdCollision(
  ...
  // 内置样板会在 refreshBuiltinDemoInDb 阶段被 bundled 重新插入 db, 所以这里
