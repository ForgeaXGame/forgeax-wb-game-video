...
import type { FXNode } from '../../react-flow-schema'
import { toFXView } from '../fx-view'
import { connect, disconnect, removeNode, setNodePosition } from '../graph-edit'
...
function PerfNode({ data }: NodeProps): JSX.Element {
  ...
  return (
    ...
    >
      {isGroup && (
        <div style={{ position: 'absolute', top: -9, right: 8, fontSize: 9, padding: '1px 6px', borderRadius: 8, background: '#f08840', color: '#0b0d10', fontWeight: 700 }}>
          ⤵ 双击下钻
        </div>
      )}
      {fx.inputs.map((h) => (
