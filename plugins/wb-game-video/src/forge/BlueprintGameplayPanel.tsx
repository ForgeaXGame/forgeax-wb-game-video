...
import type { BossRound, Branch, BranchCondition, DecisionSpec, Hotspot, PerformanceCue, SceneKind } from '../scenario/types'
 ...
 */
const KIND_OPTIONS: { kind: SceneKind; label: string }[] = [
  { kind: 'story', label: '叙事' },
  { kind: 'battle', label: 'Boss战' },
  { kind: 'qte', label: 'QTE' },
...
export function BlueprintGameplayPanel({ onCollapse }: { onCollapse?: () => void }) {
  ...
  function setDecision(patch: Partial<DecisionSpec> | null): void {
    if (patch === null) {
      updateScene(selectedSceneId, { decision: undefined })
      return
    }
  ...
  return (
    <div className={`ks-bgp ks-kind-${kind}`} data-testid="bp-gameplay-panel">
      ...
      {/* 场景类别 —— 两级状态机内层分派（见 SEEDANCE-PARITY §场景类型） */}
      <section className="ks-bgp-sec">
        <label className="ks-bgp-lbl">场景类型</label>
        <p className="ks-bgp-hint">
          决定运行时挂哪套交互层 + 蓝图节点颜色；与「界面 HUD」互补，不是重复字段。
        </p>
        <div className="ks-bgp-kinds">
          {KIND_OPTIONS.map((o) => (
            <button
              key={o.kind}
              type="button"
              className={`ks-bgp-kind ks-kind-${o.kind} ${kind === o.kind ? 'is-on' : ''}`}
              onClick={() =>
                updateScene(selectedSceneId, { kind: o.kind === 'story' ? undefined : o.kind })
              }
            >
              {o.label}
            </button>
      ...
      {/* 选项 —— 对齐原型「选项 / 限时 QTE」组 */}
      <section className="ks-bgp-sec">
        <label className="ks-bgp-lbl">选项</label>
        <select
          className="ks-bgp-input"
        ...
        {scene.decision?.optType === 'timed_qte' ? (
          ...
          <>
            ...
            {(scene.decision.optType === 'timed' || scene.decision.mode === 'timed' || scene.decision.mode === 'wait') && (
            ...
            />
            <p className="ks-bgp-hint">
              选项列表 = 连到本节点的 choice 分支（{scene.branches.filter((b) => b.kind === 'choice').length} 条）
            </p>
          </>
        ) : null}
        ...
        {scene.decision?.optType === 'timed_qte' && (
          <p className="ks-bgp-hint">
            判定结果：{scene.branches.filter((b) => b.kind === 'qte_pass' || b.kind === 'qte_fail').map((b) => b.kind).join(' / ') || '请连 qte_pass / qte_fail 分支'}
          </p>
        )}
      </section>

      <BranchConditionsSection
      ...
      {kind === 'battle' && (
        <section className="ks-bgp-sec">
          <label className="ks-bgp-lbl">Boss 战</label>
                  ...
                  <div className="ks-bgp-rounds">
                    {(scene.boss.rounds ?? []).map((r, i) => (
                      <div key={r.id} className="ks-bgp-round">
                        ...
                        <input
                          ...
                          title="命中→Boss 伤害"
                          placeholder="→Boss"
                          value={r.damageToBoss ?? ''}
                          onChange={(e) =>
                            setRounds(
                              scene.boss!.rounds.map((x) =>
                                x.id === r.id
                                  ? { ...x, damageToBoss: e.target.value ? Number(e.target.value) : undefined }
                                  : x,
                              ),
                            )
                          }
                        ...
                        <input
                          ...
                          title="失手→玩家 伤害"
                          placeholder="→我"
                          value={r.damageToPlayer ?? ''}
                          onChange={(e) =>
                            setRounds(
                              scene.boss!.rounds.map((x) =>
                                x.id === r.id
                                  ? { ...x, damageToPlayer: e.target.value ? Number(e.target.value) : undefined }
                                  : x,
                              ),
                            )
                          }
                    ...
                    <button
                      ...
                      onClick={() =>
                        setRounds([
                          ...(scene.boss?.rounds ?? []),
                          { id: `r${Date.now().toString(36)}`, damageToBoss: 25, damageToPlayer: 20 },
                        ])
                      }
                    >
                      + 添加回合
      ...
      {/* 标记 —— 对齐原型 setFlags */}
      <section className="ks-bgp-sec">
        <label className="ks-bgp-lbl">标记</label>
        {(scene.setFlags ?? []).map((flagId, i) => (
          <div key={`${flagId}-${i}`} className="ks-bgp-row">
            <input
              className="ks-bgp-input"
              type="text"
              placeholder="标记 varId"
              value={flagId}
              onChange={(e) => {
                const next = [...(scene.setFlags ?? [])]
                next[i] = e.target.value
                updateScene(selectedSceneId, { setFlags: next.filter(Boolean).length ? next : undefined })
              }}
            />
            <button
              type="button"
              className="ks-bgp-round-del"
              onClick={() => {
                const next = (scene.setFlags ?? []).filter((_, j) => j !== i)
                updateScene(selectedSceneId, { setFlags: next.length ? next : undefined })
              }}
            >
              ✕
            </button>
        ...
        <button
          type="button"
          className="ks-bgp-add"
          onClick={() => updateScene(selectedSceneId, { setFlags: [...(scene.setFlags ?? []), ''] })}
        >
          + 添加标记
        </button>
      </section>
      ...
      {/* 视频生成入口（配置 → 添加到对话，由 Nodia 触发 gvid:generate-video） */}
      <VideoGenSection
...
/**
 * 演出结算轴 —— 对齐 seedance attackBeat / 原型属性组。
 */
function PerformanceSection({
  ...
  cues: PerformanceCue[]
  onChange: (cues: PerformanceCue[]) => void
}) {
  function patch(i: number, p: Partial<PerformanceCue>): void {
    onChange(cues.map((c, j) => (j === i ? { ...c, ...p } : c)))
  }

  return (
    <section className="ks-bgp-sec">
      <label className="ks-bgp-lbl">结算时间轴</label>
      <p className="ks-bgp-hint">到时刻对 Boss/玩家扣血（FMV 回合演出，非独立 Boss 战层）。</p>
      {cues.map((c, i) => (
        <div key={c.id} className="ks-bgp-hotspot">
          <div className="ks-bgp-row">
            <input
              className="ks-bgp-input"
              type="number"
              placeholder="时刻 ms"
              value={c.atMs}
              onChange={(e) => patch(i, { atMs: Number(e.target.value) || 0 })}
            />
            <input
              className="ks-bgp-input"
              type="number"
              placeholder="→Boss"
              value={c.damageToBoss ?? ''}
              onChange={(e) =>
                patch(i, { damageToBoss: e.target.value ? Number(e.target.value) : undefined })
              }
            />
            <input
              className="ks-bgp-input"
              type="number"
              placeholder="→玩家"
              value={c.damageToPlayer ?? ''}
              onChange={(e) =>
                patch(i, { damageToPlayer: e.target.value ? Number(e.target.value) : undefined })
              }
            />
          </div>
          <input
          ...
          />
          <button type="button" className="ks-bgp-round-del" onClick={() => onChange(cues.filter((_, j) => j !== i))}>
            删除
          </button>
        </div>
      ))}
      <button
        ...
        onClick={() =>
          onChange([
            ...cues,
            { id: `perf-${Date.now().toString(36)}`, atMs: 1000, damageToBoss: 20 },
          ])
        }
      >
        + 添加结算点
...
/** 选项分支出现条件 —— 对齐原型 opt 出现条件（标记已记/未记）。 */
function BranchConditionsSection({
  branches,
  flagVars,
  onPatchBranch,
}: {
  ...
  onPatchBranch: (branchId: string, patch: Partial<Branch>) => void
}) {
  if (branches.length === 0) return null

  function readFlagCond(b: Branch): { mode: 'always' | 'set' | 'unset'; varId: string } {
    const flag = b.condition?.all?.find((c) => c.type === 'flag')
    if (!flag || flag.type !== 'flag') return { mode: 'always', varId: '' }
    return { mode: flag.equals ? 'set' : 'unset', varId: flag.varId }
  }

  function writeFlagCond(b: Branch, mode: 'always' | 'set' | 'unset', varId: string): BranchCondition | undefined {
    if (mode === 'always' || !varId) return undefined
    return { all: [{ type: 'flag', varId, equals: mode === 'set' }] }
  }

  return (
    <section className="ks-bgp-sec">
      <label className="ks-bgp-lbl">分支出现条件</label>
      {branches.map((b) => {
        const fc = readFlagCond(b)
        return (
          <div key={b.id} className="ks-bgp-hotspot">
            <span className="ks-bgp-hint">{b.label ?? b.id}</span>
            <select
              className="ks-bgp-input"
              value={fc.mode}
              onChange={(e) =>
                onPatchBranch(b.id, {
                  condition: writeFlagCond(b, e.target.value as 'always' | 'set' | 'unset', fc.varId),
                })
              }
            >
              <option value="always">始终出现</option>
              <option value="set">标记已记</option>
              <option value="unset">标记未记</option>
            </select>
            {fc.mode !== 'always' && (
              <select
                className="ks-bgp-input"
                ...
                onChange={(e) =>
                  onPatchBranch(b.id, {
                    condition: writeFlagCond(b, fc.mode, e.target.value),
                  })
                }
              >
                ...
                {!flagVars.some((v) => v.id === fc.varId) && fc.varId && (
                  <option value={fc.varId}>{fc.varId}</option>
                )}
              </select>
            )}
          </div>
...
/**
 * HotspotsSection —— 蓝图节点热区列表（detour / 子流程）。
 */
function HotspotsSection({
  ...
  function patch(i: number, p: Partial<Hotspot>): void {
    onChange(hotspots.map((h, j) => (j === i ? { ...h, ...p } : h)))
  }

  return (
    <section className="ks-bgp-sec">
      <label className="ks-bgp-lbl">热区</label>
      {hotspots.map((h, i) => (
        <div key={h.id} className="ks-bgp-hotspot">
          <input
            className="ks-bgp-input"
            type="text"
            placeholder="id"
          ...
          <textarea
            className="ks-bgp-input"
            rows={2}
            placeholder="detour 对话（每行一句，填了则不走子流程）"
            value={h.detour?.dialogue.join('\n') ?? ''}
            onChange={(e) => {
              const lines = e.target.value.split('\n').map((s) => s.trim()).filter(Boolean)
              patch(i, {
                detour: lines.length ? { ...h.detour, dialogue: lines, speaker: h.detour?.speaker } : undefined,
                targetSceneId: lines.length ? undefined : h.targetSceneId,
              })
            }}
          />
          {!h.detour?.dialogue?.length && (
            <select
              className="ks-bgp-input"
              value={h.targetSceneId ?? ''}
              onChange={(e) => patch(i, { targetSceneId: e.target.value || undefined })}
            >
              <option value="">子流程入口 scene…</option>
              {sceneIds.map((id) => (
                <option key={id} value={id}>
                  {id}
                </option>
          ...
          <label className="ks-bgp-check">
            <input
              type="checkbox"
              checked={!!h.once}
              onChange={(e) => patch(i, { once: e.target.checked || undefined })}
            />
            只触发一次
          </label>
          <button
...
.ks-bgp-kinds { display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; }
.ks-bgp-kind {
...
.ks-bgp-kind.is-on { color: #fff; font-weight: 600; }
.ks-bgp-kind.ks-kind-story.is-on  { background: rgba(148,163,184,0.3); border-color: #94a3b8; }
.ks-bgp-kind.ks-kind-battle.is-on { background: rgba(239,68,68,0.3); border-color: #ef4444; }
.ks-bgp-kind.ks-kind-qte.is-on    { background: rgba(34,211,238,0.3); border-color: #22d3ee; }
.ks-bgp-kind.ks-kind-choice.is-on { background: rgba(245,158,11,0.3); border-color: #f59e0b; }
.ks-bgp-input {
  width: 100%;
  padding: 6px 8px;
