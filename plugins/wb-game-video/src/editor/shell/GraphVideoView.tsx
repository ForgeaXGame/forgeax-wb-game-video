...
export function GraphVideoView(): JSX.Element {
  ...
  // 右列：库（添加控件）/ 检视器（素材属性）。选中控件→inspector；未选中→library。
  const [topPanel, setTopPanel] = useState<'library' | 'inspector'>('library')
  ...
  function patchMaterial(item: MaterialItem, patch: { startMs?: number; endMs?: number; zIndex?: number }): void {
    editScenario((s, n) => patchMaterialGraph(s, n, maxMs, item, patch))
  }
