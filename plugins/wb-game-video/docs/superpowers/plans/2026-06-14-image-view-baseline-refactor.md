...
**Architecture:** 工作仓库 = `forgeax-studio/packages/marketplace/plugins/wb-reel`（即 `forgeax-marketplace` 仓库工作副本）。改后须 `bun run build` 重建 dist（host 优先加载 dist）。先 bug 修复（crossPaneSync 加 imageSection），再纯数据层（presets + store action + 过滤纯函数，全 TDD），最后 UI 组件（PosterCarousel + 各分区）。
