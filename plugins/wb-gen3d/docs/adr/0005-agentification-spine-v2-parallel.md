...
1. **agent 化不是翻 `exposedToAI` boolean 就完成。** 工具进入某 agent 的 LLM 工具清单需要：工具
   ...
   **且** host-tools 桥真的注入。核实：当前**无任何 agent 声明 `gen3d:*`**；`exposedToAI` 在
   ...
   > （commit `8b86459`）落地于 `builtin/kits/host-tools/plugins/host_tool_bridge.ts`（`:149-151` 按
...
1. **发现①（桥状态）已更正 → 桥早已存在且在用**：`packages/server/builtin/kits/host-tools/plugins/host_tool_bridge.ts`
