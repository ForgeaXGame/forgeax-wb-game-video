...
- A 线第一步（PLAN §2.A0）是 **host-tools 桥 de-risk probe**：先证"白名单 + exposedToAI ⇒
  工具真进 agent LLM 清单"。桥若缺失，则"实现注入桥"是 server 改动、**需 operator 授权**。
...
- **代价/风险**：① 若 A0 发现桥缺失，agent 化需要一次 operator 授权的 server 改动，A 线变长；
