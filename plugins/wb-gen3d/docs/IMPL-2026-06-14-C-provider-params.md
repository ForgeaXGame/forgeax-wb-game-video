...
**Conventions:** All commands run from `packages/marketplace/plugins/wb-gen3d/`. `bun test` needs the runner from Plan A P1.2 Step 2b (`"test": "bun test"` + tsconfig `"exclude": ["**/*.test.ts"]`); if not present, add it first (Task C0). After `src/**` changes, `bun run build` + hard-refresh Workbench.
