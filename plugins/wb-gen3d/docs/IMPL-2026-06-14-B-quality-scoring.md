...
**Conventions:** All commands run from `packages/marketplace/plugins/wb-gen3d/`. `bun test` requires the test runner wired in Plan A Task P1.2 Step 2b (`"test": "bun test"` in `package.json` + `"exclude": ["**/*.test.ts"]` in `tsconfig.json`); **if Plan A has not run, do that wiring as the first step here** (Task B0). After `src/**` changes, `bun run build` + hard-refresh the embedded Workbench.
