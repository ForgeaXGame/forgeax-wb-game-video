// Export one asset (its main GLB + every same-basename sidefile: rig/motion
// GLB+FBX, textures, preview) plus a manifest.json into a single .zip the user
// can hand off. Pure front-end: each file is fetched from the existing
// /api/game-assets route (resolved by blobUrl) and zipped in the browser, so no
// server route, tool, or dependency is added.
...
// "assets/3d/characters/knight.glb" → "knight". The asset's main-GLB stem is
// already sanitized (PerGameAssetStore.sanitizeBaseName), so it is safe as both
// the zip's root folder and the download file name.
