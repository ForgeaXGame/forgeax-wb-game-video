...
- **`forge-script` 路径**：锻造管线**已内置**"提取锚点 + 锚点出图"（全量扩写后自动回填稀疏/缺失的 `characters/locations/props`，蒸馏人物关系，并在图像服务可用时自动出定妆照/场景基准图/道具图）。所以经 `reel:forge-script` 起的剧本**默认就有**拆好的角色/场景/道具，不必再手动拆。只有当图像服务是 Mock（锚点拆了但没出图）时，配好图像服务后再补一次 `reel:generate-visuals({})` 出图即可。
- **叙事导入路径**：经 `reel:import-from-narrative` 进来的剧本只带场景+对白、**没有**锚点出图这一步，**必须由你显式触发**一次，否则作者在视觉页签什么都看不到（角色没定妆照、场景没基准图、道具没图）：
  - 时机：M4 剧本/分镜导入完成后，调用一次 `reel:generate-visuals({})`
