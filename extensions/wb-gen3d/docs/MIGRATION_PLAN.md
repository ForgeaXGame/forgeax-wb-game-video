...
- Initial implementation boundary: `packages/marketplace/extensions/wb-gen3d/`
...
- **Embedded same-origin serving** (2026-06-10): the workbench now loads inside
  Studio from `/extensions/wb-gen3d/` (manifest `entry.standalone.embeddedAlso:true`),
...
- Embedded in Studio (2026-06-10): the iframe resolves to `/extensions/wb-gen3d/`
  (same-origin, `readyState=complete`), both left/center panes render the real UI;
  `/extensions/wb-gen3d/` + its hashed JS both return HTTP 200 from the host server.
...
and `/extensions/wb-gen3d/*` (the built workbench dist, same-origin). These are the
