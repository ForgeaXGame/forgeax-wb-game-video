...
- `L0` = `packages/marketplace/extensions/<id>/`(打包进仓库,所有人都看到)
- `L1` = `~/.forgeax/extensions/<id>/`(用户全局,不入仓)
- `L2` = `<projectRoot>/.forgeax/extensions/<id>/`(本项目私有,Git-friendly)
...
得到答案后,定下:
- 目录: 通常 `<projectRoot>/.forgeax/extensions/<slug>/`
...
2. 目录不在 `.forgeax/extensions/<id>/` 下(L2 必须正好在 project 根的 `.forgeax/extensions/`)
...
- [ ] `POST /api/extensions/reload` 后 `loaded` 含你的 plugin id
...
- 真实复杂例子: `packages/marketplace/extensions/wb-character/`(workbench kind, 12 个 tools)
