...
export class PlatformBridge {
  ...
  private handleMessage(msg: unknown): void {
    ...
    if (m.type === 'STUDIO_INIT') {
      // forgeax-studio variant: payload-less, ctx at top level. Old workbench
      // variant: { type, payload:{ filePath } }. Keep both branches.
      ...
      // host pushed a slug change without re-init
      ...
      // AI tool / host button => invoke a pipeline by id. skill-editor
      // exposes window.__ceInvoke for this; falling back to a generic event so
      // listeners (e.g. UIManager) can act on tool-ids unknown to the
      // registry.
