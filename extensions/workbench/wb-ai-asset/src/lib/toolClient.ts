// Thin client over the Studio tools router (POST /api/tools/call). When embedded
// in Studio the dist is served same-origin, so the relative path reaches the
// server directly. Standalone dev relies on the vite proxy.
//
...
// Auto-inject the active game slug into every call's args (per-game asset ops).
// provider-status ignores it; store tools require it. An explicit args.slug wins.
