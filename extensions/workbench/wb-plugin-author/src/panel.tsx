 ...
 *
 * Minimal in-app editor for L2 plugins under <projectRoot>/.forgeax/extensions/<slug>/.
 *
 *   - Left:  flat file tree from GET /api/extensions/files?slug=<slug>
 *   - Right: textarea editor; save -> PUT /api/extensions/files (server reloads
 *     the registry in the background); manual reload via POST /api/extensions/reload
 *
...
const PROJECT_ROOT_HINT =
  'Reads/writes under .forgeax/extensions/<slug>/. slug is the directory name produced by record-as-skill or fork.';
...
export function PluginAuthorPanel(): ReactElement {
  ...
  async function loadList(slug: string) {
    ...
    setEntries([]);
    try {
      const r = await fetch(`/api/extensions/files?slug=${encodeURIComponent(slug)}`);
  ...
  async function loadFile(slug: string, path: string) {
    ...
    try {
      const r = await fetch(
        `/api/extensions/files?slug=${encodeURIComponent(slug)}&path=${encodeURIComponent(path)}`,
      );
      const j = (await r.json()) as
  ...
  async function save() {
    ...
    try {
      const r = await fetch('/api/extensions/files', {
        method: 'PUT',
  ...
  async function reload() {
    ...
    try {
      const r = await fetch('/api/extensions/reload', { method: 'POST' });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--fx-border, #2a2a2a)', display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <button
          ...
          style={{ padding: '4px 10px', background: 'var(--fx-bg-elev1, #2a2a2a)', color: 'var(--fx-fg, #ddd)', border: '1px solid var(--fx-border, #444)', borderRadius: 3, fontSize: 12, cursor: 'pointer' }}
          title="POST /api/extensions/reload"
        >
