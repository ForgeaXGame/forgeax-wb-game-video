...
// asset's files (main GLB + rig/motion FBX/GLB + textures + preview + manifest)
// into a single .zip for handoff. Store (method 0) is intentional: GLB/FBX and
// PNG/WebP are already compressed, so deflate buys almost nothing and store
// keeps this tiny and dependency-free. Not ZIP64 — guarded below (a character
// bundle is far under the 4GB / 65535-entry limits).

export interface ZipEntry {
  // Path inside the archive (forward slashes), e.g. "knight/knight.glb".
  name: string;
  data: Uint8Array;
}
