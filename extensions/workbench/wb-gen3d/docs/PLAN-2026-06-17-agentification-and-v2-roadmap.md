...
**本轮边界（沿用工作区硬规则）:** 默认只动 `packages/marketplace/extensions/wb-gen3d/` + 新增 `agent-gen3d` 插件目录。**A5（引擎加载）和任何 `packages/server`/`packages/engine` 改动跨插件边界，必须先拿 operator 授权**（见 §0.2 / §2.A5）。
...
forgeax-extension.json  provides.agent.tools: ["gen3d:*"]
  ...
  └─→ host_tool_bridge.ts   builtin/kits/host-tools/extensions/
...
**Files（只读参考，不改）:**
- `packages/marketplace/extensions/agent-reel-storyboard/forgeax-extension.json:26`（`provides.agent.tools` 模板，A1 直接抄）
- `packages/server/builtin/kits/host-tools/extensions/host_tool_bridge.ts:149-151,175`（桥实现，了解即可）
...
**Files:**
- Create: `packages/marketplace/extensions/agent-gen3d/forgeax-extension.json`
- Create: `packages/marketplace/extensions/agent-gen3d/persona/zh.md`
- Create: `packages/marketplace/extensions/agent-gen3d/memory/`（空目录占位，放 `.gitkeep`）
- Reference: `packages/marketplace/extensions/agent-reel-storyboard/forgeax-extension.json`（**首选模板**，`provides.agent.tools` 桥白名单工作先例）；`agent-lowpoly/forgeax-extension.json`（次选）
...
**Files:**
- Modify: `packages/marketplace/extensions/wb-gen3d/forgeax-extension.json`
...
**Files:**
- Modify: `packages/marketplace/extensions/wb-gen3d/server/tool-handlers.ts`（generate 成功落盘后调用）
- Modify: `packages/marketplace/extensions/wb-gen3d/server/per-game-store.ts`（sidecar `custom` 合并写 `qualityScore`）
...
**Files:**
- Modify: `packages/marketplace/extensions/wb-gen3d/forgeax-extension.json`
- Modify: `packages/marketplace/extensions/wb-gen3d/shared/catalog.ts`
- Modify: `packages/marketplace/extensions/wb-gen3d/docs/CAPABILITY_MATRIX.md`
...
**Files:**
- Create: `packages/marketplace/extensions/wb-gen3d/docs/v2-motion/UPSTREAM-ASK-2026-06-17.md`（结构化三问，把开放式求助变是非题）
- Create: `packages/marketplace/extensions/wb-gen3d/scripts/sanity-motion-retarget-v2.ts`（最小可复现，单发，零依赖外部状态）
...
**Files:**
- Modify/Create: `packages/marketplace/extensions/wb-gen3d/scripts/sanity-motion-retarget-v2.ts`（扩成对照矩阵）
...
**Files:**
- Create: `packages/marketplace/extensions/wb-gen3d/public/hdr/<name>.hdr`（真实 1k HDR，CC0 来源如 Poly Haven）
- Create: `packages/marketplace/extensions/wb-gen3d/public/hdr/presets.json`
