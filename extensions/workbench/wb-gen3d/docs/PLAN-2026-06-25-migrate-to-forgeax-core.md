...
- `forgeax-cli`(`packages/cli` submodule)接手 agent 运行时 + kits(含 host-tools kit:`builtin/kits/host-tools/extensions/host_tool_bridge.ts`)。

## 3 · 纠正 6/23 的一处误判

6/23 分析曾判"host-tools 桥被删 → 分支注入失效"。**错。** 桥迁进了 forgeax-cli:`packages/cli/builtin/kits/host-tools/extensions/host_tool_bridge.ts`(~210 行,逻辑近乎原样)+ `packages/cli/src/kernel/host-tool-bridge.ts`(`makeInProcessExecuteTool` + 新增 `checkKernelTool` 信任闸)。**注入机制存活**,`agent-gen3d` 的 `gen3d:*` 在新架构仍走得通。
...
已核实锚点(origin/main):
- `packages/cli/builtin/kits/host-tools/extensions/host_tool_bridge.ts`(存在;`kits.config['host-tools'].allow` 是注入开关;`delegate_to_subagent.ts:135` 也按 `persona.tools` 写 allow)。
