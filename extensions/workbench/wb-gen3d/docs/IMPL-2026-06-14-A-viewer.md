...
**Conventions:** All commands run from `packages/marketplace/extensions/wb-gen3d/`. After any `src/**` change you MUST `bun run build` to regenerate `dist/` before hard-refreshing the embedded Workbench (HANDOFF dist 铁律). A is verified by **typecheck + build + visual regression** (no unit tests — three.js/DOM), per spec §11.
