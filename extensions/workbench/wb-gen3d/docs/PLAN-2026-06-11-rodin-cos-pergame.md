...
- `packages/server/src/main.ts`：新增 `/api/game-assets/:slug/*` 静态路由(有先例：现有 `/api/gen3d-blobs` 与 `/extensions/wb-gen3d` 两块都在此处，曾获授权)。这是唯一插件外改动，开工前请确认授权；实现必须是只读预览 route，限定在 `.forgeax/games/<slug>/assets/3d/**`，不能暴露整个 game 目录。
