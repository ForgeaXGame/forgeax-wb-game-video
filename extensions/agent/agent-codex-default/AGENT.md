...
OpenAI Codex CLI 的开箱即用占位 agent。它是「把对话原样转发给 `@forgeax-extension/cli-codex`」的通用编程助手 persona —— 启动 session 时 forgeax 在用户没 pin 任何 agent 的情况下自动选用，不需要用户主动 pin。
...
- 通用编程助手，不带专属人格
- 启动时由 forgeax 自动选用，把对话原样转发给底层 cli-provider `@forgeax-extension/cli-codex`
