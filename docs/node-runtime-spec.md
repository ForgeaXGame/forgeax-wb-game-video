...
> 状态：🟢 SPEC（已定稿）· 2026-07-20 · 修订 2026-07-30
...
- **判别符 = `GameNode.type`**（不新增 `NodeData.type`）：`resolveNodeType` 现按 `subProcess/subFlowPack` 数据派生
  `perf/subProcess/subflowPack`；以后往 `GameNode.type` 加真类型是叠加式（回退 perf，届时按 AGENTS.md 征同意改 schema）。
...
- **节点只有一种 type**：`GameNodeType = 'perf'`。所谓「子流程容器 / 子蓝图容器」不是独立 type，而是
  `NodeData` 的数据变体（`subProcess` / `subFlowPack`，用 `getSubProcess` / `getSubFlowPack` / `isSubflowContainer` 读）。
- **执行流程散在引擎几个方法里，按 data 分支**（非按类型注册）：
  - `enterNode(id)`：防跑飞 → 分支（subFlowPack 下钻 / subProcess 下钻 / 容器弹回不重播）→ 普通节点：重置本节点态
...
| `subProcess`（私有内嵌子流程容器） | `{kind:'descend', entry, graph: getSubProcess(data).graph}`；弹回态 → `{kind:'advance'}` | — |
...
| `enterNode` 里 subFlowPack/subProcess 分支 | `subflowPack.execute` / `subProcess.execute` 返回 `descend` |
...
- **路径 A（当前实现）**：节点**派生**出 runtime 类型 = 引擎内一个纯函数
  `resolveNodeKind(node) → 'perf'|'subProcess'|'subflowPack'`（读 `getSubProcess`/`getSubFlowPack`），注册表按它派发。
